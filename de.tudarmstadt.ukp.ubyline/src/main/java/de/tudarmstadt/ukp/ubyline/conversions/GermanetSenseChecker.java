...
////            System.out.println("There are {" + SYNSETS_WITHOUT_DEFINITION + "} synsets without definition ");
////            System.out.println("There are {" + SYNSETS_WITHOUT_SYNONYM + "} synsets without synonym ");
////            System.out.println("There are {" + SYNSETS_WITHOUT_HYPERNYM + "} synsets without hypernym ");
...
//            SYNSETS_WITHOUT_DEFINITION = SYNSETS_WITHOUT_DEFINITION + " " + synsetID;
        ...
        if (!synset.getSynsetRelations().isEmpty()) {
            ...
            for(SynsetRelation synsetRelation : synset.getSynsetRelations()){
                ...
                String hypernym = "";
                String hyponym = "";
                if (synsetRelation.getRelName().contains("synonym")
                        && (synsetRelation.getTarget().getSenses().get(0).getLexicalEntry().getLemmaForm() != null)) {
                    synonym = synsetRelation.getTarget().getSenses().get(0).getLexicalEntry().getLemmaForm().toString();
                    synonyms = synonyms + synonym;
                    System.out.println("Synonym : " + synonym);
                    hasSynonym = true;
                }
               
                if (synsetRelation.getRelName().contains("hypernym")
                        && (synsetRelation.getTarget().getSenses().get(0).getLexicalEntry().getLemmaForm() != null)) {
                    hypernym = synsetRelation.getTarget().getSenses().get(0).getLexicalEntry().getLemmaForm().toString();
                    hypernyms = hypernyms + hypernym;
                    System.out.println("hypernym : " + hypernym);
                    hasHypernym = true;
                }
                
                if (synsetRelation.getRelName().contains("hyponym")
                        && (synsetRelation.getTarget().getSenses().get(0).getLexicalEntry().getLemmaForm() != null)) {
                    hyponym = synsetRelation.getTarget().getSenses().get(0).getLexicalEntry().getLemmaForm().toString();
...
//                SYNSETS_WITHOUT_HYPONYM = SYNSETS_WITHOUT_HYPONYM + synsetID;
    ...
    private static void checkSynset()
    {
        ...
        for (LexicalEntry lexEntry : LEXICALENTRIESWN) {
            if (!(lexEntry.getSenses().isEmpty())) {  
                int i = 1;
                for (Sense sense : lexEntry.getSenses()) {
                    System.out.println("Sense"+sense.getMonolingualExternalRefs().get(0)
    ...
    private static void importSenses(Sense sense)
    {
        ...
        Collection<FormRepresentation> lemmas = sense.getLexicalEntry().getLemma().getFormRepresentations();
        ...
        else if ((sense.getSynset().getDefinitionText()!= null)&&(sense.getSynset().getDefinitionText()!="")) {
            System.out.println("Synset: ID "+sense.getSynset().getId()+" Definition:"+sense.getSynset().getDefinitionText());
            hasDescription = true;
          
        }
        else if (sense.getSynset()!= null) {
            Collection<de.tudarmstadt.ukp.lmf.model.core.Sense> synsetSynonyms = sense.getSynset().getSenses();
...
//            }
            for (de.tudarmstadt.ukp.lmf.model.core.Sense sen : synsetSynonyms) {
                ...
                if ((sen.getLexicalEntry().getLemmaForm()!= null)&&(sen.getLexicalEntry().getLemmaForm()!="")) {
...
//        }
        ...
        if (!hasDescription) {
            NO_DESCRIPTION++;
            NOT_DESCRIBED_SENSES = NOT_DESCRIBED_SENSES+ sense.getMonolingualExternalRefs().get(0).getExternalReference()+"  ";
        }
